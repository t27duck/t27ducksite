...
**Rich text is Action Text + Lexxy.** `Page#content` and `Post#content` are both `has_rich_text`, edited with `f.rich_text_area`. `Project#description` is plain HTML in a column. The `marksmith`/`commonmarker` markdown stack has no remaining consumer and is pending removal; `commonmarker` must stay in the Gemfile permanently because three data migrations call it.
